...
router.post('/tenants', async (req, res, next) => {
  try {
    ...
    const result = await run('INSERT INTO tenants (user_id, name, phone, email, notes) VALUES (?, ?, ?, ?, ?)', [req.user.id, name, phone, email, notes]);
...
router.put('/tenants/:id', async (req, res, next) => {
  try {
    ...
    const row = await ensureOwnershipOr404(res, 'tenants', id, req.user.id); if (!row) return;
    const { name, phone = null, email = null, notes = null } = req.body;
    await run('UPDATE tenants SET name = ?, phone = ?, email = ?, notes = ? WHERE id = ? AND user_id = ?', [name || row.name, phone, email, notes, id, req.user.id]);
...
router.post('/managers', async (req, res, next) => {
  try {
    if (requireFields(res, ['name'], req.body)) return;
    const { name, phone = null, email = null, notes = null } = req.body;
    const result = await run('INSERT INTO managers (user_id, name, phone, email, notes) VALUES (?, ?, ?, ?, ?)', [req.user.id, name, phone, email, notes]);
...
router.put('/managers/:id', async (req, res, next) => {
  try {
    const id = parseId(req.params.id);
    const row = await ensureOwnershipOr404(res, 'managers', id, req.user.id); if (!row) return;
    const { name, phone = null, email = null, notes = null } = req.body;
    await run('UPDATE managers SET name = ?, phone = ?, email = ?, notes = ? WHERE id = ? AND user_id = ?', [name || row.name, phone, email, notes, id, req.user.id]);
...
router.get('/payment-methods', async (req, res, next) => {
  try { res.json(await all('SELECT * FROM payment_methods WHERE user_id = ? ORDER BY name', [req.user.id])); }
  catch (error) { next(error); }
});
router.post('/payment-methods', async (req, res, next) => {
  try {
    if (requireFields(res, ['name'], req.body)) return;
    const result = await run('INSERT INTO payment_methods (user_id, name) VALUES (?, ?)', [req.user.id, req.body.name]);
...
router.get('/receiving-accounts', async (req, res, next) => {
  try { res.json(await all('SELECT * FROM receiving_accounts WHERE user_id = ? ORDER BY name', [req.user.id])); }
  catch (error) { next(error); }
});
router.post('/receiving-accounts', async (req, res, next) => {
  try {
    if (requireFields(res, ['name'], req.body)) return;
    const result = await run('INSERT INTO receiving_accounts (user_id, name) VALUES (?, ?)', [req.user.id, req.body.name]);
...
router.put('/properties/:id', async (req, res, next) => {
  try {
    const id = parseId(req.params.id);
    const row = await ensureOwnershipOr404(res, 'properties', id, req.user.id); if (!row) return;
    const { name = row.name, address = row.address, tenant_id = row.tenant_id, manager_id = row.manager_id, rent_value = row.rent_value, notes = row.notes } = req.body;
    await run('UPDATE properties SET name = ?, address = ?, tenant_id = ?, manager_id = ?, rent_value = ?, notes = ? WHERE id = ? AND user_id = ?', [name, address, tenant_id, manager_id, rent_value, notes, id, req.user.id]);
...
router.post('/category-configs', async (req, res, next) => {
  try {
    ...
    const result = await run(
      'INSERT INTO category_configs (user_id, property_id, category_name, amount, due_day, active) VALUES (?, ?, ?, ?, ?, 1)',
      [req.user.id, req.body.property_id, req.body.category_name, req.body.amount, req.body.due_day]
    );
...
router.put('/category-configs/:id', async (req, res, next) => {
  try {
    const id = parseId(req.params.id);
    const row = await ensureOwnershipOr404(res, 'category_configs', id, req.user.id); if (!row) return;
    const { property_id = row.property_id, category_name = row.category_name, amount = row.amount, due_day = row.due_day, active = row.active } = req.body;
    await run('UPDATE category_configs SET property_id = ?, category_name = ?, amount = ?, due_day = ?, active = ? WHERE id = ? AND user_id = ?', [property_id, category_name, amount, due_day, active ? 1 : 0, id, req.user.id]);
...
router.get('/launches', async (req, res, next) => {
  try {
    ...
    if (month) { sql += ' AND l.competence = ?'; params.push(month); }
    if (manager_id) { sql += ' AND p.manager_id = ?'; params.push(parseId(manager_id)); }
...
router.post('/launches/generate', async (req, res, next) => {
  try {
    ...
    const configs = await all('SELECT * FROM category_configs WHERE user_id = ? AND active = 1', [req.user.id]);
    const created = [];
    for (const cfg of configs) {
      ...
      if (!exists) {
        ...
        const result = await run(
          'INSERT INTO launches (user_id, property_id, config_id, category_name, competence, amount_expected, due_date, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [req.user.id, cfg.property_id, cfg.id, cfg.category_name, month, cfg.amount, dueDate, null]
        );
...
router.put('/launches/:id', async (req, res, next) => {
  try {
    const id = parseId(req.params.id);
    const row = await ensureOwnershipOr404(res, 'launches', id, req.user.id); if (!row) return;
    const { amount_expected = row.amount_expected, due_date = row.due_date, notes = row.notes, category_name = row.category_name } = req.body;
    await run('UPDATE launches SET amount_expected = ?, due_date = ?, notes = ?, category_name = ? WHERE id = ? AND user_id = ?', [amount_expected, due_date, notes, category_name, id, req.user.id]);
...
router.get('/payments', async (req, res, next) => {
  try {
    const rows = await all(`
      SELECT pay.*, l.category_name, l.competence, l.amount_expected, l.due_date,
...
router.post('/payments', async (req, res, next) => {
  try {
    ...
    if (existing) {
      await run('UPDATE payments SET received_amount = ?, payment_date = ?, payment_method_id = ?, receiving_account_id = ?, rental_period_start = ?, rental_period_end = ?, notes = ? WHERE id = ? AND user_id = ?', [req.body.received_amount, req.body.payment_date, req.body.payment_method_id || null, req.body.receiving_account_id || null, req.body.rental_period_start || null, req.body.rental_period_end || null, req.body.notes || null, existing.id, req.user.id]);
      return res.json(await get('SELECT * FROM payments WHERE id = ?', [existing.id]));
    }

    const result = await run('INSERT INTO payments (user_id, launch_id, received_amount, payment_date, payment_method_id, receiving_account_id, rental_period_start, rental_period_end, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [req.user.id, req.body.launch_id, req.body.received_amount, req.body.payment_date, req.body.payment_method_id || null, req.body.receiving_account_id || null, req.body.rental_period_start || null, req.body.rental_period_end || null, req.body.notes || null]);
...
router.post('/payments/:id/receipt', upload.single('receipt'), async (req, res, next) => {
  try {
    const id = parseId(req.params.id);
    const row = await ensureOwnershipOr404(res, 'payments', id, req.user.id); if (!row) return;
    if (!req.file) return res.status(400).json({ error: 'Arquivo receipt é obrigatório.' });
    await run('UPDATE payments SET receipt_file_path = ?, receipt_original_name = ? WHERE id = ? AND user_id = ?', [`/uploads/receipts/${req.file.filename}`, req.file.originalname, id, req.user.id]);
...
router.put('/payments/:id', async (req, res, next) => {
  try {
    const id = parseId(req.params.id);
    const row = await ensureOwnershipOr404(res, 'payments', id, req.user.id); if (!row) return;
    const { received_amount = row.received_amount, payment_date = row.payment_date, payment_method_id = row.payment_method_id, receiving_account_id = row.receiving_account_id, rental_period_start = row.rental_period_start, rental_period_end = row.rental_period_end, notes = row.notes } = req.body;
    await run('UPDATE payments SET received_amount = ?, payment_date = ?, payment_method_id = ?, receiving_account_id = ?, rental_period_start = ?, rental_period_end = ?, notes = ? WHERE id = ? AND user_id = ?', [received_amount, payment_date, payment_method_id, receiving_account_id, rental_period_start, rental_period_end, notes, id, req.user.id]);
    res.json(await get('SELECT * FROM payments WHERE id = ?', [id]));
  } catch (error) { next(error); }
});
